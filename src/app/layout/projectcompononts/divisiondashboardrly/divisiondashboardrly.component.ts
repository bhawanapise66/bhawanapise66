...
@Component({
  selector: 'app-divisiondashboardrly',
  templateUrl: './divisiondashboardrly.component.html',
  styleUrls: ['./divisiondashboardrly.component.css'],
  animations: [routerTransition()]

})
export class DivisiondashboardrlyComponent implements OnInit {
 ...
 this.GetDepartmentWiseCount("")
  ...
  AdminDashboardRlyCount() {
    try {
      ...
      service.DivisionDashboardRlyCountAPI(keydata).subscribe(
        (data) => {
  ...
  try{this.admindashrlyService.AdminDashboardRlysubdivisionCountAPI(data).subscribe((data) => {
...
MapSwitch(layerindex){
 ...
 try{SwitchMap(layerindex)}catch(e){
  ...
  distanceTravelledReportPDFDownload() {
    ...
    win.document.write('<title>Dev**-***-21111*------------------------ice Report</title>');   // <title> FOR PDF HEADER.
