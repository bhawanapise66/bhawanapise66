...
@Component({
  selector: 'app-admindashboardrly',
  templateUrl: './admindashboardrly.component.html',
  styleUrls: ['./admindashboardrly.component.css'],
  animations: [routerTransition()]
})
export class AdmindashboardrlyComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    document.addEventListener("fullscreenchange", function () {
      ...
      if (document.getElementById('map').style.height == myheight + 'px') {
        ...
        if (document.getElementById("frontmap").style.transform == "rotateY(0deg)") {
          document.getElementById("frontmap").style.transform = "rotateY(180deg)";
        } else {
          document.getElementById("frontmap").style.transform = "rotateY(0deg)";
        }
        //document.getElementById("frontmap").style.transform="rotateY(180deg)";
  ...
  GetAllVehicles() {
    ...
    try {
      this.postService.GetVehicleDetailsformap(data).subscribe((data) => {
        ...
        var newarray = this.FilterDepartmentCheck(this.allvehicleData, "CENTRAL");
  ...
  CheckSection(type) {
    try { this.ClearAll() } catch (e) { }
    var checkid = this.Splitsectionid(type);

    if ($("#" + checkid).prop('checked') == true) {

      var data = this.allvehicleData.filter(x => x.param59 == type);

      data.forEach(element => this.sectiondevices.push(element));

    } else {
      this.sectiondevices = this.FilterDepartmentCheck(this.sectiondevices, type)
    }
    ...
    console.log("the final array lenght is    " + this.sectiondevices.length)

    try { this.CreateTable(this.sectiondevices) } catch (e) { }
  ...
  OpenFullScreen() {
    ...
    //qw2112zzzzthis.CentralFilter("mapsection");
    document.getElementById("frontmap").style.transform = "rotateY(0deg)";
    // try{openFullscreen(elem)}catch(e){}
  ...
  PrepareExcelData(data) {
    ...
    for (var i = 0; i < data.length; i++) {
      var obj = {
        ...
        "Sub Division": data[i].param55,
        "Department/Section": data[i].param59,
  ...
  distanceTravelledReportPDFDownload() {
    ...
    win.document.write('<title>Devices Report</title>');   // <title> FOR PDF HEADER.
  ...
  GetDepartmentWiseCount(subdiv) {
    ...
    try {
      this.admindashrlyService.RailwayDepartmentWiseCount(data).subscribe((data) => {
  ...
  GetSubdivision() {
    ...
    try {
      this.admindashrlyService.AdminDashboardRlysubdivisionCountAPI(data).subscribe((data) => {
        ...
        this.subdivisionlist = data.entity.list;
      })
  ...
  SectionFilter(type) {
    ...
    this.sectionsummary = this.sectioncount.filter(city => city.param10 == type);
  ...
  FilterDepartmentCheck(arr, value) {

    return arr.filter(function (ele) {

      return ele.param59 != value;
    });
