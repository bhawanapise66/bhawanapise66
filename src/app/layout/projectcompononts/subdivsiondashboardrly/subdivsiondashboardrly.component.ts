...
declare var intervalrail: any; declare var SetrailwayGauge: any; declare var railwayGuage: any;
...
@Component({
  selector: 'app-subdivsiondashboardrly',
  templateUrl: './subdivsiondashboardrly.component.html',
  styleUrls: ['./subdivsiondashboardrly.component.css'],
  animations: [routerTransition()]
})
export class SubdivsiondashboardrlyComponent implements OnInit {
  ...
  AdminDashboardRlyCount() {
    try {
      ...
      service.SubDivisionDashboardRlyCountAPI(keydata).subscribe(
        (data) => {
          ...
          for (var i = 0; i < this.devisionData.length; i++) {
            ...
            nopower = nopower + Number(this.devisionData[i].param15);
            medium = medium + Number(this.devisionData[i].param13);
            lowbattery = lowbattery + Number(this.devisionData[i].param14);
            high = high + Number(this.devisionData[i].param12);
  ...
  GetAllVehicles() {
    ...
    try {
      this.postService.GetVehicleDetailsformap(data).subscribe((data) => {
        ...
        this.allvehicleData = data.entity.list;




        this.PrepareExcelData(this.allvehicleData)


        // this.exampleData=data.entity.list;
        // this.options={
        //    /// templateResult: this.templateResult,
        //  templateSelection: this.Select2Display
        // }
  ...
  DivisionsTotalVehicles(type) {
    ...
    if (type == "All") {
      ...
      this.filterVehicles = this.filterVehicles.filter(data => data.param59 == type);
      try { this.CreateTable(this.filterVehicles) } catch (e) { }
  ...
  DivisionsTotalVehiclesfilter(type, subtype) {
    ...
    if (type == 'All') {
      ...
      this.filterVehicles = this.filterVehicles.filter(data => data.param59 == type && data.param31 == subtype);
