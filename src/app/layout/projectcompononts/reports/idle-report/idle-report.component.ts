...
@Component({
  ...
  styleUrls: ['./idle-report.component.css'],
  animations: [routerTransition()]

})
export class IdleReportComponent implements OnInit {
  loginRoleId: any; isCustomer: boolean = false;
  vehiclesavailable: boolean = false;

  customerId: string;

  fromDate: string; toDate: string; StartTime: string; endTime: string;
  filter = '';
  ...
  EncryptPageName() {
    this.cryptService.encrypt("idle report")
  ...
  onSubmit() {
    ...
    if ((this.isCustomer == false) && (this.customerId == null || this.customerId == '' || this.deviceIdArr == [])) {
      ...
      this.submitted = false; this.ValueAlreadyGot = false; this.IdleReport()
    }
  }
...
AddLoader()
    this.reportService.railVehiclesummaryReport(dataL).subscribe((response) => {
      RemoveLoader()
      if (response.statuscode == 200) {
        this.PrepareExcelData(response.entity);
      }
    })
  }
  ...
  exportToExcel() {
    this.excelservice.ExportExcel(this.excelData, 'Idle Report', 'idlereport');
  }

  VehicleSummaryReportForPDF() {
    let dataL = {
      param1: this.customerId,
      selectvehicleid: this.deviceIdArr,// this.deviceIdArr,
      param3: this.fromDate,
  ...
  exportToPDF() {
    this.pdfservice.CreatePDFData(this.pdfData, 'Idle Report');
  }
  ...
  Openmap(data) {
    ...
    // this.historyservice.HistoryPlot(data.param2, data.param3, this.historyData.fromdate, this.historyData.fromtime, this.historyData.todate, this.historyData.totime, '0', '0', data.param17, staticdetails);
    this.historyservice.HistoryPlot(data.param2, data.param3, fromDate, fromTime, toDate, toTime, 0, 0, "Idle", staticdetails)
