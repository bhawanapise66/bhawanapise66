...
@Component({
  ...
  styleUrls: ['./duty-report.component.css']
})
export class DutyReportComponent implements OnInit {
  loginRoleId: string;
  ...
  getKeymanReport() {
    ...
    AddLoader()
    this.reportService.RailKeymanPatrolmanReport(dataL).subscribe((response) => {
      RemoveLoader()
      if (response.statuscode == 200) {
        this.keymanReportArray = response.entity;
  ...
  PrepareExcelData(data) {
    ...
    for (var i = 0; i < data.length; i++) {
      var obj = {
        "#": data[i].rowNumber,
        "NAME OF KEYMAN/PATROLMAN": data[i].param3,
        "FROM LOCATION": data[i].param6,
        "TO LOCATION": data[i].param7,
        "ALLOWED DISTANCE": data[i].param8,
        "FROM DUTY HOUR": data[i].param9,
        "TO DUTY HOUR": data[i].param10,
        ...
        "NO OF TRIPS COMPLETED ": data[i].param19,
        "STATUS ": data[i].param20,
        "STOP PAGE DETAILS ": data[i].param21,
      }
  ...
  exportToExcel() {
    try {
      this.excelservice.ExportExcel(this.excelData, 'Keyman/Patrolman Report', 'keymen/partolmanreport');
    }
    catch (e) {
  ...
  exportToPDF() {
    try {
      this.pdfService.CreatePDFData(this.pdfData, 'Keyman/Patrolman Report');
    }
    catch (e) {
      alert("on button function" + e)
    }
