...
@Component({
  selector: 'app-keyman-exception-report',
  templateUrl: './keyman-exception-report.component.html',
  styleUrls: ['./keyman-exception-report.component.css']
})
export class KeymanExceptionReportComponent implements OnInit {
  loginRoleId: string;
  ...
  constructor(private excelservice: ExportToExcelService, private router: Router, private pdfservice: PdfService, private listService: ListService, private cryptService: CryptService, private historyservice: HistoryService, private reportService: ReportService) {
  ...
  PreparePdfData(data) {
    ...
    for (var i = 0; i < data.length; i++) {
      var obj = {
        ...
        "ACTUAL DISTANCE": data[i].param14,
  ...
  exportToPdf() {
    try {
      this.pdfservice.CreatePDFData(this.pdfData, 'Keyman Exeption Report');
    }
    catch (e) {
  ...
  reportExcel() {
    ...
    this.reportService.RailwayExceptionReport(dataL).subscribe((response) => {
      RemoveLoader()
      if (response.statuscode == 200) {
        let exceldata = response.entity.responsedatalist;
  ...
  exportToExcel() {
    try {
      this.excelservice.ExportExcel(this.excelData, 'Keyman Exeption Report', 'keymannexceptionreport');
    }
    catch (e) {
      alert("on button function" + e)
    }
  ...
  //   const startdate = startDateTimeArray[0]; const starttime = startDateTimeArray[1] + " " + startDateTimeArray[2]
  //   const enddate = endDateTimeArray[0]; const endtime = endDateTimeArray[1] + ' ' + endDateTimeArray[2]
  ...
  //     param6: endtime,
  //     param7: "All",
  //     param8: "exceptionkeyman",
  //     pageID: "",
  //     pageName: this.encryptedpageNameValue,
  //     pageURL: this.encryptedpageUrlValue
  //   }
  //   AddLoader()
  //   this.reportService.GeofenceReport(georeqparam).subscribe((response) => {
  //     RemoveLoader()
  //     if (response.statuscode == 200) {
