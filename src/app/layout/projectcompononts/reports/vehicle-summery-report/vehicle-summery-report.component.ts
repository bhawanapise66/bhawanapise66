...
@Component({
  selector: 'app-vehicle-summery-report',
  templateUrl: './vehicle-summery-report.component.html',
  styleUrls: ['./vehicle-summery-report.component.css'],
  animations: [routerTransition()]

})
export class VehicleSummeryReportComponent implements OnInit {
  ...
  encryptedpageNameValue: string; encryptedpageUrlValue: string; totalrecord = "NA";
  ...
  onSubmit() {
    ...
    if ((this.isCustomer == false) && (this.customerId == null || this.customerId == '' || this.deviceIdArr == [])) {
      ...
      this.submitted = false; this.ValueAlreadyGot = false; this.VehicleSummaryReport()
    }
  ...
  exportToPdf() {
    this.pdfservice.CreatePDFData(this.pdfData, 'Vehicle Summary Report');
  }
  ...
  exportToExcel() {
    this.excelservice.ExportExcel(this.excelData, 'Vehicle Summary Report', 'vehiclesummaryreport');
  }
  ...
  deviceSummeryReportPDFDownload() {
    var sTable = document.getElementById('deviceSummeryPDF').innerHTML;
    var style = "<style>";
    style = style + "table {width: 100%;font: 13px Calibri;}";
    style = style + ".table, .headings>th, .trheight> td {border: solid 1px #DDD; border-collapse: collapse;";
    style = style + "padding: 2px 3px;text-align: center;}";
    style = style + "#invisible {padding-left: 4%;padding-right: 4%;border:none}  ";
    style = style + ".w{width: 140px;border:2px solid darkblue;border-radius:5px}";
