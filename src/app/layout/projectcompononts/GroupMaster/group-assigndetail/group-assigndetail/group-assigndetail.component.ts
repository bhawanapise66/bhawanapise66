...
@Component({
  selector: 'app-group-assigndetail',
  templateUrl: './group-assigndetail.component.html',
  styleUrls: ['./group-assigndetail.component.css']
})
export class GroupAssigndetailComponent implements OnInit {
  ...
  EncryptPageName() {
    this.cryptService.encrypt("Assign Vehicle To Group Details")
  ...
  itemsPerPage: number; p: number; groupVehicleDetails: any[] = []; count: number; viewcount: number; actualdata: any;
  typeidArray = [];
  GroupVehicleDetail() {
    ...
    this.groupservice.VehicleAssignToGroupDetail(keydata).subscribe(
      (data) => {
        ...
        // console.log(this.groupVehicleDetails[0]["param8"]);

        // dataArr = JSON.parse(this.groupVehicleDetails[0].param8)


        //    console.log(dataArr[0]['f1'])

        // var ar =this.groupVehicleDetails[i].param8;
        // console.log(ar);
        //       for(var i=0; i< this.groupVehicleDetails.length; i++)
        //       {
        //         var ar =this.groupVehicleDetails[i].param8;
        //         console.log(ar);
        // dataArr = JSON.parse(this.groupVehicleDetails[i].param8);
        // console.log("data user:"+dataArr);
        // this.dataArr1.push(dataArr[0]);
        //     }


        //   console.log(this.groupVehicleDetails["0"]["param8"][0]["f1"]);
  ...
  GroupVehDeletefunction() {
    //  this.deleteAssign();
    var isValid = true;

    var deleteremark = $('#grpdelremark').val();
    ...
    if (!deleteremark && deleteremark.length <= 0) {
      ...
      this.groupservice.VehicleUnAssignToGroup(dataL).subscribe((data) => {
        try { RemoveLoader() } catch (e) { alert(e) }
  ...
  exportToExcel() {
    ...
    this.excelservice.ExportExcel(this.excelData, 'Assign Vehicle To Group Details', 'assignvehiclegroupdetails');
  }
  ...
  createPDF() {
    ...
    if (this.flag1 == 1) {
      for (let i = 0; i < this.excelpdfDetails1.length; i++) {
        pdfTableData = {
          "#": i + 1,
          "Customer Name": this.excelpdfDetails1[i]["param4"],
          "Group Name": this.excelpdfDetails1[i]["param9"],
          "Group Description": this.excelpdfDetails1[i]["param10"],
          "No. of Vehicles": this.excelpdfDetails1[i]["param3"],
          "Assign Date": this.excelpdfDetails1[i]["param2"],
        }
    ...
    this.pdfservice.CreatePDFData(dataArray, "Assign Vehicle To Group Details");
    ...
    if (this.flag1 == 1) {
      for (var i = 0; i < data.length; i++) {
        ...
        var obj = {
          ...
          "#": i + 1,
          "Customer Name": data[i].param4,
          "Group Name": data[i].param9,
          "Group Description": data[i].param10,
          "No. of Vehicles": data[i].param3,
          "Assign Date": data[i].param2,
        }
        // } catch (e) { }
  ...
  GrpVehpageChanged(event) {
    this.p = event; this.pagecount = $("#selectgrpveh").val();
