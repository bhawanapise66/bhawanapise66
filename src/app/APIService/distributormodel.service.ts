...
Date      	: 
Description : Distributor Module Api Service
...
export class DistributormodelService {

  constructor(private http: HttpClient) { }

  UpdatedistributorAPI(param){
    ...
    return this.http.post<any>(URL + 'updatedistributor', myObjStr, httpOptions) 
   }
   DistributorDetailsAPI(param){
    //alert('hi');
    const myObjStr = JSON.stringify(param);
    var httpOptions = {
      headers: new HttpHeaders({
  ...
  return this.http.post<any>(URL + 'distributordetails', myObjStr, httpOptions) 
 }
 InsertdistributorAPI(param){
  const myObjStr = JSON.stringify(param);
  var httpOptions = {
    headers: new HttpHeaders({
      'HeaderKey': sessionStorage.getItem('hk'),
  ...
  return this.http.post<any>(URL + 'insertdistributor', myObjStr, httpOptions) 
 }
 DeleteDistributorAPI(param){
  const myObjStr = JSON.stringify(param);
  var httpOptions = {
    headers: new HttpHeaders({
      'HeaderKey': sessionStorage.getItem('hk'),
  ...
  return this.http.post<any>(URL + 'deletedistributor', myObjStr, httpOptions) 
 }
}
